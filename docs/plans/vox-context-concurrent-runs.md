...
- [ ] Stage 3 — Web diplomacy/chat. **Cache-correctness core landed:** `GameState.eventsAfter` records the lower event-ID bound a slice was fetched with; `ensureGameState()` is now coverage-aware (a cached entry is a hit only when `eventsAfter <= parameters.after`, i.e. it covers the requested range — `undefined` means legacy/sufficient), and `refreshGameState()`'s same-turn in-place merge keeps the *wider*-covering slice (smaller `after` wins, serialized size only as a tiebreak at equal coverage). This closes the cache-hit race where a chat refreshing the live turn narrowly (`after = turn*1e6`) would let a lagging strategist (`after = its event cursor`, reaching back across a dropped turn) short-circuit its wider fetch and lose the dropped turn's events. `/agents/message` now composes a run-local view at the session's live turn (`currentTurnOf`) for game-state prep + execution, returning an explicit error when a live session has no turn. Tests: `strategy-parameters.test.ts` (coverage-aware hit/miss + wider-wins merge in both arrival orders). **Remaining:** the full web-route `withRun()` migration (SSE `streamProgress` via `VoxRunOptions`, per-run disconnect cancellation via `run.abort()`, `respondToHumanDeal()` own run), and bounding the `civIdentity`/`currentTurnOf` display helpers by the live session turn.
