...
- The option landscape comes from `get-options`, not the panel. On every decision turn the panel is handed the same `get-options` payload the LLM's context is built from — the currently valid options for each action type, each explained with the **same descriptive text the LLM prompts already use**. (Mechanically the `human-strategist` triggers the hand-off but the payload is fetched server-side by the `present-decision` tool rather than round-tripped through the agent — see the implementation plan; either way the panel receives the identical landscape.) The panel holds no game logic; it renders exactly the option set and guidance the model would receive, no more and no less. This same hand-off is also how the panel learns a decision is due (see §4).
