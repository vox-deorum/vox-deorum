...
- The option landscape comes from the strategist, not the panel. The `human-strategist` already fetches `get-options` on every decision turn — the same call the LLM's context is built from — and hands that payload to the panel: the currently valid options for each action type, each explained with the **same descriptive text the LLM prompts already use**. The panel holds no game logic; it renders exactly the option set and guidance the model would receive, no more and no less. This same hand-off is also how the panel learns a decision is due (see §4).
...
- On the human civ's decision turns, the strategist pauses the human's civ at a safe point (the existing bridge pacing) and signals the in-game panel — handing over the turn's `get-options` payload — so the panel knows a decision is pending and what the options are. The game stays paused until the human submits a decision or explicitly keeps the status quo. There is no timeout; sessions are supervised.
...
- Decisions leave the game the way render events already do: the panel fires a fire-and-forget `Game.BroadcastEvent`, which travels the existing DLL → bridge → MCP notification path (the only addition is registering the human-decision event in the MCP notification whitelist — no new channel is built). The waiting `human-strategist` receives it and issues the same MCP action tools an LLM would. The existing pause machinery does the waiting, the existing action tools do the acting.
...
- **`vox-agents`** — the `human-strategist` agent and the session-config plumbing: a human-control mode built on top of observe mode (auto-play everything, but animations on, observer UI off, pause at the human's decision points); signalling the panel when a decision is due and handing it the turn's `get-options` payload; decision wait/submit handling; recording simple wall-clock deliberation time.
...
- **`mcp-server`** — nearly unchanged: the human's decisions execute through the same action tools, and the decision event reaches the strategist through the existing event-notification path; the one addition is registering the human-decision event in the notification whitelist. The existing heartbeat (already used before long synchronous blocks) keeps the SSE channel alive across an unbounded human pause.
