...
- **The inbound decision path reuses the notification channel.** `Game.BroadcastEvent` → DLL → bridge → the MCP store's `handleGameEvent` → `sendNotification` → the vox-agents MCP client → the `StrategistSession` notification switch. The notification keeps canonical routing metadata (`event`, `playerID`, `turn`, `latestID`) at top level and carries the validated event payload inside a top-level `data` object, so the human's choices ride the notification without being spread over routing fields.
- **The outbound "present decision" path reuses the `LuaFunction` pattern** from `mcp-server/src/utils/lua/player-actions.ts`: the `present-decision` tool fetches the turn's options via `get-options` and hands them to a preregistered Lua function that receives JSON-serialized string arguments over the bridge batch queue (never inline-interpolated into a script), firing `LuaEvents.VoxDeorumHumanDecision(...)` into the panel.
...
3. **payload forwarding** — the store now forwards validated event data for whitelisted events inside the notification's top-level `data` object instead of spreading event fields over the notification metadata;
