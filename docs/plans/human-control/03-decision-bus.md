...
2. **`vox-agents/src/strategist/agents/human-strategist.ts`** — flesh out the stage 1 stub. `getSystem(parameters, _input, context)` (the three-argument `VoxAgent` form):
   1. `ensureGameState` for the turn's state (normally already cached by `VoxPlayer`).
   2. Call `present-decision` with the player, turn, mode, and the state's `OptionsReport`.
   ...
   5. Map the submission onto action tools, replicating the single rationale across every call (spec §2/§3). A `keepStatusQuo` submission calls `keep-status-quo` with the **human's actual rationale** — never the `"[skipped]"` sentinel, which tells that tool to refresh AI settings *without* recording a decision (it's what `VoxPlayer`'s paced-skip path sends); a human keep-status-quo IS a decision and must be recorded as one. Otherwise: Strategy mode → `set-strategy` plus `set-research` / `set-policy` (and `set-persona` / `set-relationship` once those panel sections exist); Flavor mode → `set-flavors` instead of `set-strategy`. Each tool fires only when the panel actually submitted that field.
   6. Record deliberation time twice, paralleling token usage: as an attribute on the active turn span (the turn's trace is the active OpenTelemetry context inside `getSystem`), and as an accumulated per-player total written via `set-metadata` (key `deliberationMs-<playerID>`, overwritten with the running total each decision — idempotent and crash-safe), paralleling the `inputTokens-<playerID>` totals `VoxPlayer` writes. Keep the running total in `parameters.workingMemory`.
...
4. **`vox-agents/src/strategist/vox-player.ts`** — accept the session's bus as a constructor argument and expose it as `parameters._humanDecisionBus` (populated for every seat; only the human strategist reads it).
...
With a session running: the strategist logs that it is presenting a decision and blocks (no turn advance). A synthetic `HumanDecision` injected via `lua-executor` — once with `keepStatusQuo`, once with a full sample decision — routes to the right action tools (replay messages appear), the game resumes, and deliberation time is recorded in both slots. Kill the game mid-wait: the session recovers and the decision is re-presented.
