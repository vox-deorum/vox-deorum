...
6. **`mcp-server/src/tools/actions/present-decision.ts`** (new) — a tool taking `{PlayerID, Turn, Mode, Options}` that JSON-stringifies `Options` and calls the `presentHumanDecision` Lua function. Register it in `tools/index.ts` alongside the other action tools.
...
- **Inbound:** with a session running, use `lua-executor` to fire `Game.BroadcastEvent("HumanDecision", {PlayerID=<id>, Turn=..., Rationale="test", keepStatusQuo=true})`; the vox-agents log shows the notification with `event:"HumanDecision"` and the choice fields intact.
