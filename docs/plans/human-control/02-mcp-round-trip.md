...
> 1. **Notification payload forwarding is generic, not a `HumanDecision` special-case.** [`store.ts`](../../../mcp-server/src/knowledge/store.ts) now forwards the validated event `data` for *every* whitelisted event, not just `HumanDecision`. [`server.ts`](../../../mcp-server/src/server.ts) no longer spreads that payload into notification params; it sends `{ event, playerID, turn, latestID, data }`, with the event-specific payload nested under `data`. Capitalized event fields (`data.PlayerID`/`data.Turn`/...) ride along unchanged, while accidental or malicious lowercase payload keys cannot overwrite routing metadata.
...
2. **`mcp-server/src/knowledge/store.ts`** — in `handleGameEvent`, forward the validated event data as the payload argument to `sendNotification` so the human's choices ride the `HumanDecision` notification. *(As built, the final notification call passes `data` for every whitelisted event rather than special-casing `HumanDecision` — see Deviation 1.)*
