...
**Staging shim (Stages 1–4, removed in Stage 5 — done).** So the run-model core could land and be reviewed before callers migrated, `execute()`/`callAgent()` kept their original signatures (with the `parameters` argument) during the intermediate stages. When called outside an active run they wrapped themselves in an _ephemeral root_ over the supplied `parameters`, so every existing caller kept working unchanged. Likewise `lastParameter` was a getter/setter alias of `currentParameters`/`setBaseParameters`, and `streamProgress`/`timeoutRefresh` kept a context-field fallback for callers that still assigned them before opening a run. **Stage 5 removed all of this:** `execute()`/`callAgent()` now _require_ an active run (rejecting otherwise) and no longer take a `parameters` argument; the ephemeral-root path, the legacy `_legacyStreamProgress`/`_legacyTimeoutRefresh` fields, and `lastParameter` are gone.
...
- [x] `strategist/pacing/important-events.ts` (`flattenEvents(state.events)`) — slice reader; **keep `state.events`**. Pacing computes importance to _decide_ the window, so it must read the immutable per-turn slice, not the merged window. Verify and comment this intent.
- [x] `strategist/strategy-parameters.ts` `mergeCachedEvents()` (`gameStates[turn]?.events`, `report.events`) — slice reader; **keep `state.events`**. This is the source that _builds_ `mergedEvents` from each turn's immutable slice.
