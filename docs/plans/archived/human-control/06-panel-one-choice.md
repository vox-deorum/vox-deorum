...
- **The native screens key off the active player.** `TechTree.lua`(`g_activePlayerID = Game.GetActivePlayer()`) and `SocialPolicyPopup.lua`(`Players[Game.GetActivePlayer()]`) would render the _observer's_ empty research/policy state, not the human civ's. Launches load Vox Populi + EUI, so the EUI variants (`civ5-dll/UI_bc1/...`) are the ones in effect.
...
On a decision turn the human picks a research target from the in-panel list; `set-research` fires with the human's rationale; the replay message appears; the game resumes.
