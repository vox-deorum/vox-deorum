...
- The human occupies a strategist seat: their decisions are _influence_ over the in-game AI (see the MCP server's influence model), never direct orders to units or cities.
...
- **The view is pinned to the human's civ.** Community Patch's "AI takeover" feature gives us the core behavior: setting the observer-UI override player (`Game.SetObserverUIOverridePlayer`, available from Lua) before auto-play begins makes the observer slot copy that civ's team visibility plot by plot at activation, and from then on mirror its sight, revealed tiles, met-civilization status, and notification queue. Vox Deorum adds a narrow DLL compatibility patch for native reward popups that still key directly off the active player, such as ancient ruin rewards. Human-control mode launches like observe mode today, with the override set to the human's civ before `SetAIAutoPlay` — the ordering matters, because the initial visibility copy happens only when auto-play activates.
...
- Deliberation time per human decision is recorded by the in-game panel as simple wall-clock — from the first time the human opens the decision dialog until they submit. Hiding and reopening the dialog does not reset the timer, and no OS-level focus tracking is done.
...
- **`vox-agents`** — the `human-strategist` agent and the session-config plumbing: a human-control mode built on top of observe mode (auto-play everything, but animations on, observer UI off, pause at the human's decision points); signalling the panel when a decision is due (via `present-decision`, which supplies the turn's `get-options` payload); decision wait/submit handling; recording the panel-reported deliberation time.
...
- **`civ5-dll`** — small UI compatibility changes: the observer-UI override that pins the auto-play view to one civ's fog of war already exists in Community Patch (`ObserverUIOverridePlayer`, set via `Game.SetObserverUIOverridePlayer`, serialized in saves), but Vox Deorum adds the narrow reward-popup compatibility needed for pinned-civ ancient ruin rewards to render from the observer slot. Stage 7 also bumps the connection-service buffer size so the full `OptionsReport` always fits through the bridge's `lua_call` path.
