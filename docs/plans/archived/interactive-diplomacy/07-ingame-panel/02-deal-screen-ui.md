...
The stage-7.02 mock driver has six FireTuner scenarios: `author`, `incoming`, `own`, `error`, `unavailable`, and `coop-war`. It uses no legality bypass. Each ordinary-item probe uses the DLL sentinels `GOLD(amount,-1,-1)`, `RESOURCES(resourceID,quantity,-1)` with duration supplied only in Add, `OPEN_BORDERS(duration,-1,-1)`, `ALLOW_EMBASSY(-1,-1,-1)`, `GPT(amount,duration,-1)`, and `MAPS(-1,-1,-1)`. It probes cumulatively, adds a candidate, immediately calls `AreAllTradeItemsValid(true)`, and rejects then rebuilds the aggregate when that check fails. Military, Expansion, and Border promises are selected only when their guarded live getter reports no active commitment, then the mock falls back to structurally legal No Digging. Invalid additions appear only in `unavailable`. Coop War selects the first fully legal third major target and does not open when none exists. Stage 7.04 replaces the mock include only.
...
10. Test author, incoming, and own states with normal play and a pinned human strategist. Run the unchanged normal-seat mock in a pure-observer session only to confirm presentation; it is not proof of live deal support. Confirm the effective seat supplies participants, legality, value, presentation columns, and serialized actor for supported seats. Confirm accepting the incoming mock closes after the delayed successful result, mock error preserves the mounted editor, and neither produces bridge traffic or native enactment.
