...
5. **Stored deal moves in the transcript** — proposal and counter messages carry the proposed terms in `Payload.Deal`, plus optional proposal-time `Payload.Value1` / `Payload.Value2` snapshots. Accept/reject/enacted messages carry `Payload.ProposalMessageID`. Current legality and enactment checks still come from fresh `inspect-deal` / `enact-agent-deal` calls; `enact-agent-deal` itself is idempotent (it refuses a second enactment of a proposal ID that already has a `deal-enacted`).
