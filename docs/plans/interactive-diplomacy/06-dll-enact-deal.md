...
4. **No enum/save change; version bump.** No `TradeableItems` addition, no serialization change, no new acceptability/valuation logic. The only edit to an existing function is the defaulted `bHumanToHuman` parameter on `AreAllTradeItemsValid` (stage 3 did the same for `IsPossibleToTradeItem`) — backward-compatible, stock behavior preserved. Bump the DLL version and rebuild (`scripts/release.py`, `CustomMods.h` `MOD_DLL_VERSION_NUMBER`).
