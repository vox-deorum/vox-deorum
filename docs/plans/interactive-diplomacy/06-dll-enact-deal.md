...
5. **`mcp-server`** — an `inspect-deal`-adjacent **`enact-agent-deal`** tool (non-read-only, `ActionTool` + a `LuaFunction` wrapper) that takes the full `Deal` object directly, calls `inspect-deal` fresh for live validation/diagnostics, then calls `EnactAgentDeal` with the trade items **and** the promise commitment list. It does **not** reduce transcript state or accept proposal IDs. Register the factory in `tools/index.ts`.
6. **Caller orchestration after acceptance** — Web UI / vox-agents reduce the append-ordered transcript to confirm the current proposal is accepted, disable or guard duplicate UI actions, call stateless `enact-agent-deal` with the full deal object, then append a `deal-enacted` transcript message with `Payload.ProposalMessageID` on success. This keeps enactment pure and leaves stateful workflow policy in the surfaces that own it.
...
Via `lua-executor` and end to end through stages 2–5: enact a complete deal object the **stock AI would refuse on political grounds** — items change hands for real. A structurally-illegal item is rejected with a reason (the always-on `IsPossibleToTradeItem` guards still apply). A bad promise in an otherwise valid item trade rejects before any trade item changes hands. A deal carrying a promise (and one carrying Coop War against a third party) writes real diplomacy state, and the promise thereafter **behaves like an in-game promise** — e.g. broken by a later declaration of war. End to end, the Web/agent caller appends one `deal-enacted` message after success and guards against duplicate UI submission; `enact-agent-deal` itself remains stateless. The **normal in-game deal pathway and AI valuation behave exactly as before**; the agent path is a separate entrypoint with no `TradeableItems`/save-format change.

## Done when

Both sides agreeing in a Web conversation results in the accepted deal object being **enacted for real** — trade items and any of the nine promises — through the additive, `MOD_ACTIVE_DIPLOMACY`-gated entrypoint, with structurally-illegal items still rejected, a `deal-enacted` archive message appended by the caller on success, and the stock pathway untouched. **This completes human↔LLM Web v1** (specs § Success criteria).
