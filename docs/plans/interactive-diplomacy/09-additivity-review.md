...
- **The one shared-function edit is backward-compatible.** `IsPossibleToTradeItem` (stage 3) and `AreAllTradeItemsValid` (stage 6) each gain a single *defaulted* `bHumanToHuman` parameter. Confirm: (a) the default value reproduces the prior computed `bHumanToHuman` (`... || bHumanToHuman` at `CvDealClasses.cpp:362`, default-`false`), so every stock caller that passes nothing behaves identically; (b) no other logic in those functions is changed; (c) the only callers passing `true` are the agent inspection wrapper and `EnactAgentDeal`. This is the single spot to keep merge-aware against upstream Vox Populi.
