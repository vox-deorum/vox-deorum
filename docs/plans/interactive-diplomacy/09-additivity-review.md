...
- **Agreement and enactment orchestration are explicit.** The transcript reducer identifies the current agreed proposal without a status column. Web/agent callers guard duplicate submission, call stateless `enact-agent-deal` with the full deal object, and append `deal-enacted` on success. `enact-agent-deal` does not read transcript state or accept proposal IDs.
