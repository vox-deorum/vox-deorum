...
- **The shared legality/reason edits are backward-compatible.** `IsPossibleToTradeItem` (stage 3) and `AreAllTradeItemsValid` (stage 6) each gain a single *defaulted* `bTreatAsHumanToHuman` parameter, and the inspection reason path uses the same semantics. Confirm: (a) the default value reproduces the prior computed `bHumanToHuman` (`computedHumanToHuman || bTreatAsHumanToHuman` at `CvDealClasses.cpp:362`, default-`false`), so every stock caller that passes nothing behaves identically; (b) no other stock-path logic in those functions is changed; (c) the only callers passing `true` are the agent inspection wrapper and `EnactAgentDeal`. This is the main spot to keep merge-aware against upstream Vox Populi.
