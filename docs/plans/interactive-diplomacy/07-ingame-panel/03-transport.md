...
6. **`vox-agents/src/envoy/ingame-bridge.ts` — the bridge skeleton.** A new module owned by `StrategistSession` (instantiated beside `humanDecisionBus`; new cases in the `mcpClient.onNotification` switch, `strategist-session.ts:188-230`). This stage implements:
   - **Per-pair promise-chain FIFO** — all handling for a pair is serialized (MCP notification dispatch is fire-and-forget), and the FIFO doubles later as the ordered push queue. **Inbound dedupe** on the generated event id (pipe + SSE fallback can double-deliver).
   - **`DiplomacyPanelOpened`** → read-only reflush: `readTranscriptPage` (never `openDiplomacyChat` — it compacts/re-hydrates the live thread cache and would corrupt an in-flight turn) → push `Begin{hasEnvoy, busy, hasMore}` + ordered `Messages{append}` batches packed to the ~32 KB wire budget via `call-lua-function`. `hasEnvoy` from the seat's assignment/live context; `busy` from item 7.
   - **`DiplomacyTranscriptRequest`** → `readTranscriptPage(BeforeID)` → `Messages{prepend, hasMore}`.
   - **`DiplomacyChatMessage`** → a **temporary probe handler** (append via the transcript utils + echo the committed row back as a `Messages` append). Marked loudly in code: it is **deleted and replaced by `runChatTurn`** in stage 04 (`runChatTurn` commits internally; keeping this would double-append).
   - **Delimiter sanitization** (`!@#$%^!` stripped) on all pushed `Content`.
   - `DiplomacyDealAction`: acknowledged with an error `Status` ("not wired yet") — wired in stage 04.
...
5. Duplicate delivery: re-emitting the same generated event id is ignored; two rapid `PanelOpened` events for one pair execute serially (FIFO).
