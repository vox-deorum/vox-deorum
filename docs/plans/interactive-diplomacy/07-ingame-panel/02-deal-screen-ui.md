...
6. **Serialization and stub emits.** Propose and Counter serialize `draftItems` and `draftPromises` directly into `DealPayload` v1. Do not serialize the shared scratch deal. Immediately before submission, run `rebuildScratchDeal()`, then call the same human-to-human legality and promise checks used while editing. Refuse any locally invalid or oversized payload with an on-screen reason. Accept and Retract carry the mock `proposalMessageID`; Retract emits `reject`. All actions end at stubbed emits (print plus mock toast). The screen never calls `Deal:Enact()` locally because enactment belongs to stage 04. Choose the response buttons from the opener's mode: incoming, own, view-only, or authoring.
...
The full deal-editing experience works in-game against mock data: incoming proposals, pockets using enactment legality, live balances, and promises. It produces the exact `DealPayload` v1 objects that stage 04 will send.
