...
2. **`civ5-mod/UI/VoxDeorumConverse.lua/.xml`: the Converse button and shipped embedding retrofit.** Track the on-screen leader through `Events.AILeaderMessage`; argument 1 is `diploPlayerID`. Registration and eligibility are not the current visibility failure: both LeaderHeadRoot variants load the `DiplomacyUIAddin`, and `canConverse` passes for a normal human player. The shipped `ContextPtr:LookUpControl("../DiscussButton")` lookup is the failing gate because `DiscussButton` sits below the named `RootOptions` container in both control trees.
   ...
   If none of the ID paths reaches `DiscussButton`, look up `RootOptions`, reparent the button there, and anchor it explicitly beside the action controls. Log once that the unnamed wrapper prevented action-stack discovery and record the resulting layout limitation. Add the button without changing native buttons. On click, run the (3a) close sequence (`UI.SetLeaderHeadRootUp(false); UI.RequestLeaveLeader()`) and fire `LuaEvents.VoxDeorumDiploOpen(m_diploPlayerID)`. Show it only for living major civilizations eligible under the normal-player and observer rules in [specs.md](specs.md).
...
4. **`civ5-mod/UI/VoxDeorumDiploPanel.lua/.xml`: the mock-driven chat panel.** Follow the `VoxDeorumHumanPanel` lifecycle: hide the `InGameUIAddin` at load, show and hide it explicitly outside the popup stack, and handle Esc locally. Draw it full-screen over the native diplomacy background, with a transparent main container instead of a second framed window.
   ...
   - **Declare War**: fully functional already with no bridge. Follow the button-existence rule in [specs.md](specs.md), then gate against peace and declaration legality on the effective seat's team. Normal play keeps `CanDeclareWar(counterpartTeam)` and `Network.SendChangeWar(counterpartTeam, true)`. A human strategist gates with `Teams[pinnedTeam]:CanDeclareWar(counterpartTeam, pinnedSeatID)`, then uses `Teams[pinnedTeam]:DeclareWar(counterpartTeam, false, pinnedSeatID)`, with all three declaration arguments explicit, so passive-mode and event-hook checks, the attacker team, and originator and warmonger attribution all use the pinned seat. A pure observer has no Declare War button. The button disappears once at war.
...
5. **Observer retrofit for the shipped stage.** Treat this as retrofit work on the already implemented stage rather than a new UI surface. Use the observer flavors, effective-seat definition, thread identities, identity convention, and button-existence rule pinned in [specs.md](specs.md).
   ...
   - Apply the capability matrix from [specs.md](specs.md): Propose Deal and Declare War exist only for a human-controlled effective seat. Hide both for a pure observer, render deal cards view-only, and leave accept, counter, and reject unavailable. For a human strategist, compute war availability with `Teams[pinnedTeam]:CanDeclareWar(counterpartTeam, pinnedSeatID)` and use the pinned-seat `Teams[pinnedTeam]:DeclareWar(counterpartTeam, false, pinnedSeatID)` branch described above. Normal play retains its existing gate and `Network.SendChangeWar` path.
   - Extend `VoxDeorumDiploPanelMock.lua` with a pure-observer scenario that proves the Observer title, chat rendering, absent seat-holder actions, and view-only deal cards without requiring a live observer game.

### Panel layout (text mockup)

The full-screen panel uses the native diplomacy backdrop with a transparent content layer, leaving the conversation bubbles and footer controls visually dominant. Messages use @schnetziomi5's dark bubble design, with the speaker portrait on the outside corner. Counterpart messages sit left, player messages sit right, and centered turn pills separate turns.
...
1. For a normal human player, the leader screen shows **Converse** alongside Discuss/Trade/Demand/War, all natives still functional. Clicking it leaves the leader scene, posts **one native notification**, and opens the panel on the mock conversation for that leader. This confirms that add-in registration and `canConverse` were already working and that the embedding retrofit fixed the visibility gate.
...
7. In the pure-observer mock, Converse and chat remain available, the own speaker is Observer, deal cards are view-only, and Propose Deal, Declare War, accept, counter, and reject are absent.
...
A normal human player or human strategist in a live game can walk the whole mock-backed conversation surface: leader screen → Converse → notification → panel with correctly derived deal cards and visible progress states. The pure-observer mock proves the chat-only surface and its seat-holder action exclusions.
