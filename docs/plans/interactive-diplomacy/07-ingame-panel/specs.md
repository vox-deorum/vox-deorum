...
- **Local draft and legality, server enactment.** The scratch deal is the live editing truth: TradeLogic's in-place seams apply h2h semantics to legality reads, every native `Add*` constructor, and — through the extended DLL bindings — the three native amount-change methods. `draftItems` is a passive decode of the scratch after every native refresh; `draftPromises` never enters the scratch; serialization always builds from the draft plus promises plus the outgoing message. The screen implements both `DealPayload` to scratch and scratch to `DealPayload` mappings, and rebuilds the scratch from the draft in exactly three cases: on open, on a throttled fingerprint mismatch (external clobber restore that never rolls back a legitimate native edit), and before final validation and broadcasting. Stage 7.04 extends the shared proposal writer to reject structurally illegal inspected promises before archival; stage 6 rechecks everything before enactment. Server-side tools may reuse the scratch deal without destroying the draft. A pending Propose or Counter keeps the editor mounted; an error restores controls without reinitializing the draft. Enactment and every transcript write stay server-side. The mod never writes transcript rows; `deal-accept` and `deal-enacted` still come only from `enact-agent-deal`.
...
- The shared scratch deal: `UI.GetScratchDeal()` is one global object. A server-side `inspect-deal` or enact call can replace it between UI interactions. The draft is decoded after every native refresh; the scratch is rebuilt from it on open, on a throttled fingerprint mismatch, and before serialization.
