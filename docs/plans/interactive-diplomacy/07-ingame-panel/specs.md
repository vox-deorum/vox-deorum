...
The in-game counterpart of the Web v1 diplomacy flow for three active-player situations: a **normal human player** playing their own civilization, a **human strategist** observing with `Game.GetObserverUIOverridePlayer()` pinned to their civilization seat, and a **pure observer** with no override. The effective seat governs the conversation. Normal humans and human strategists receive the full diplomat experience. Pure observers keep the spokesperson voice and lack seat-holder authority, but fresh live attestation grants them full deal interaction through their concrete non-negative observer slot. The surface includes a **Converse** entry on the leader discussion screen, a native-styled chat panel over the durable pair transcript with streamed replies, a vendored copy of the game's own trade screen with locally computed legality and values plus the promises category, and async reply delivery through the game's **native notification system** so a conversation can stretch across turns ("mailing"). Deals enact through the same `enact-agent-deal` path as the Web. Pure observers cannot declare war.
...
Caller authorization never trusts the client-supplied `PlayerID`, `AsObserver`, or display identity by itself. Before any mutation, a read-only mcp-server seat-state query reads the game's active player, observer status, and observer UI override, then derives the live effective seat and pure-observer flag from the same rule as Lua. The bridge requires the event fields to match that live state. A pure observer must identify the active concrete observer slot and set `AsObserver: true`; normal humans and pinned strategists must identify the live effective seat and omit the field. `isSeatHolder` stays false for a pure observer and may continue to gate non-deal authority. `canActOnDeals` is true for all three valid live states. The fixed `{ name: 'Observer' }` identity and `civIdentity` are presentation data, never proof of authority. Inconsistent or stale caller state fails closed before chat, greeting, or deal mutation.
