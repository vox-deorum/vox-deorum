...
**Capability rule:** normal humans and human strategists have `canActOnDeals: true`. A pure observer has it only when its concrete slot is a participant that the native `CvDeal` path supports. TradeLogic's pre-mutation `VoxDeorumOpenDeal` range gate is authoritative: a native participant satisfies `0 <= playerID < GameDefines.MAX_MAJOR_CIVS`. The mcp-server seat-state field must mirror that exact definition when deriving `canActOnDeals`. An out-of-range observer slot has `canActOnDeals: false`, so the panel leaves deal controls unavailable and the bridge rejects deal actions before mutation. Declare War remains a separate seat authority and is never available to a pure observer.
...
| Deal cards and deal actions | Full interaction | Full interaction on behalf of the pinned civilization | Readable; interaction only when the concrete slot is native-supported |
| Propose Deal | Present | Present | Present only when the concrete slot is native-supported |
...
When `AsObserver` is true and live seat attestation confirms pure observation, the bridge uses the event `PlayerID` as `callerPlayerID`, forces `agentName: 'spokesperson'`, and passes `callerRole: 'Observer'` and `{ name: 'Observer' }` as the caller identity. It grants `canActOnDeals` only when that concrete slot is native-supported. Deal turns may use the negotiator handoff described above only in that case. When `AsObserver` is absent and attestation confirms a normal or pinned seat, the effective seat receives the normal diplomat and deal behavior. The bridge always passes the event `PlayerID` explicitly because `resolveHumanSeat` cannot infer a seat in observer or autoplay sessions.
...
- **Live effective-seat attestation and observer limits.** Add a read-only mcp-server query for the current active player, observer state, override, effective seat, pure-observer flag, and whether the concrete slot satisfies TradeLogic's authoritative `0 <= playerID < GameDefines.MAX_MAJOR_CIVS` participant gate. The bridge uses it before mutations and derives `canActOnDeals` from the mirrored result. Transcript append may admit exactly the live pure-observer slot, with role `Observer`, paired with a valid major civilization. Deal inspection and enactment must retain the native participant limit: an out-of-range observer slot is unsupported and its deal actions fail before the deal path. Keep the Web's `-1` sentinel behavior unchanged, including its existing transcript-only semantics. Visibility remains limited to real major-civilization columns; the out-of-range observer slot is part of the ordered pair but does not create a visibility column.
...
- **Proposal review, local editing, and server enactment.** Proposal cards and the `incoming` and `own` review modes render the immutable original `DealPayload` directly. Scratch projection may validate those ordinary terms transactionally, but it never supplies proposal presentation. Accept and Counter gate on native structural legality and promise validation of the original proposal. Counter then enters the counter editor, which owns a separate draft and submits `{ kind: 'deal', deal, expectedProposalID }`. The scratch deal is used to validate ordinary items through native code and to edit author and counter drafts: TradeLogic's in-place seams apply h2h semantics to legality reads, every native `Add*` constructor, and the three native amount-change methods. In editor states, `draftItems` is a passive decode of the scratch after native editing refreshes; `draftPromises` never enters the scratch; serialization builds from the draft plus promises plus the outgoing message. Scratch projection verifies the normalized draft before final validation and broadcasting. Stage 7.04 extends the shared proposal writer to reject structurally illegal inspected promises before archival; stage 6 rechecks everything before enactment. Server-side tools may reuse the scratch deal without destroying an editor draft. A pending Propose or Counter keeps the editor mounted; an error restores controls without reinitializing the draft. Enactment and every transcript write stay server-side. The mod never writes transcript rows; `deal-accept` and `deal-enacted` still come only from `enact-agent-deal`.
...
- The shared scratch deal: `UI.GetScratchDeal()` is one global object. A server-side `inspect-deal` or enact call can replace it between editor interactions. The editor decodes its draft after native refreshes and projects it again before final validation and serialization. Proposal review never depends on the scratch.
