...
The in-game counterpart of the Web v1 diplomacy flow supports three active-player situations: a **normal human player** playing their own civilization, a **human strategist** observing with `Game.GetObserverUIOverridePlayer()` pinned to their civilization seat, and a **pure observer** with no override. The effective seat governs the conversation. All three get chat, streaming, notifications, and every deal action. A pure observer differs in two ways: it speaks with the built-in `spokesperson` voice through its concrete observer slot, and it can never declare war. The surface includes a **Converse** entry on the leader discussion screen, a native-styled chat panel over the durable pair transcript with streamed replies, and the game's own trade screen with locally computed h2h legality plus promises. The preferred trade-screen path includes VP logic unchanged; a uniquely named fallback copy is allowed only if the stage-7.02 seam probes fail. Native notifications let a conversation stretch across turns ("mailing"). Deals enact through the same `enact-agent-deal` path as the Web.
...
- **Local draft and legality, server enactment.** Plain Lua `draftItems` and `draftPromises` own serialization and clobber recovery. The scratch proxy applies h2h semantics to legality reads and all native `Add*` constructors; the three native amount-change methods update the draft and rebuild through those adapters because their bindings have no h2h option. Two-way synchronization uses a rebuild guard, native-mutation marker, and expected scratch fingerprint so an external replacement is restored from the draft without rolling back a legitimate native edit. The screen implements both `DealPayload` to scratch and scratch to `DealPayload` mappings. It rebuilds before final validation and broadcasting. Stage 7.04 extends the shared proposal writer to reject structurally illegal inspected promises before archival; stage 6 rechecks everything before enactment. Server-side tools may reuse the scratch deal without destroying the draft. A pending Propose or Counter keeps the editor mounted; an error restores controls without reinitializing the draft. Enactment and every transcript write stay server-side. The mod never writes transcript rows; `deal-accept` and `deal-enacted` still come only from `enact-agent-deal`.
...
- VFS rename discipline: any copied file must be renamed (`import="1"` same-name files silently override VP's originals for all native consumers). Stage 02 avoids the hazard entirely for the trade logic by including `TradeLogic` in place instead of vendoring it; the rule still binds its XML copy and any fallback copy.
...
- **Instance definitions for the copied trade-screen XML**: all templates the native trade logic needs (`CityInstance`, `PocketResource`, `PocketTechnology`, `PocketVote`, `TableStrategic`, `TableLuxury`, `TableTechnology`, `TableVote`, `OtherPlayerEntry`) are defined inside `DiploTrade.xml`, so the copied XML carries them automatically. Stage 7.02 adds separate wrapper-owned templates for promise entries.
