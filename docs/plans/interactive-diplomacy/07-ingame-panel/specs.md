...
2. **The server pushes rows, not state.** Exactly like the Web — which derives deal status client-side by reducing the message list (`deriveActiveProposal`) — the panel receives transcript rows and derives everything else in Lua. The only pushed non-row facts are things no client can derive: `hasEnvoy`, `busy`, `hasMore`.
...
There is deliberately no state/status push beyond these: reflushes and increments carry rows, and the panel derives deal state from them (below). A worked example — reject: the human clicks the active proposal bubble, which opens the deal screen in respond mode, and clicks Reject there → back in the panel the bubble greys with an animated "rejecting ⋯" row → `DiplomacyDealAction{reject, ProposalMessageID}` → the bridge runs the shared reject action (thread lock, open-proposal guard, `appendDealReject`) → pushes `Messages{append: [the deal-reject row]}` → the panel appends the row as an ordinary bubble, re-runs the reducer — the proposal reverts to view-only — and the pending state clears because its matching row arrived. Identical in shape to the Web rendering a new message.
...
`NOTIFICATION_VOX_DEORUM_DIPLOMACY`, a data-driven row in the `Notifications` XML table — `NotificationTypes` is populated from that table at load, so **no DLL change creates the type**, and EUI's NotificationPanel renders unknown types with its `Generic` instance (our summary/message strings become the tooltip).

- **Posting**: mod-registered `VoxDeorumPostNotification(playerID, counterpartID, summary, message)` → `Players[playerID]:AddNotificationName("NOTIFICATION_VOX_DEORUM_DIPLOMACY", message, summary, -1, -1, counterpartID, counterpartID)` (`iGameDataIndex` carries the counterpart for click routing). Invoked locally for the stage-01 smoke test and via `call-lua-function` from the bridge in stage 04.
- **Click dispatch**: there is no `Events.NotificationActivated` anywhere in civ5, and the DLL's `Activate` switch default is a no-op — so a small guarded branch in the owned fork's NotificationPanel.lua (`GenericLeftClick`) fires `LuaEvents.VoxDeorumDiplomacyNotificationActivated(Id, counterpartID, extra)` for our type and returns. Nil-guarded so vanilla installs are inert.
- **Cross-turn persistence**: `CvNotifications::IsNotificationTypeEndOfTurnExpired` defaults to `true` for unknown types (auto-dismissed at the turn boundary) — one additive early-out returns `false` for our hashed type. Save persistence is already free (`Notification::Serialize`).
...
- `openDiplomacyChat(request)` (`web/chat/factory.ts`) — deterministic thread per pair (`dipl:${gameID}:${min}:${max}`), diplomat resolution from `assignments[target].diplomat`, identity handling. Used by the bridge to ensure the thread before a mutating action — never for read-only reflushes (it compacts/re-hydrates the live cache).
...
**Extended (small, shared with the Web routes — not bridge-private forks):**
...
- **A game `ChatStreamSink`.** The only existing sink is the SSE one; stage 04 adds the in-game adapter (delta throttling, status allowlist, push-queue ordering).

**New (no Web counterpart exists):**

- The four game-event schemas + whitelist entries; the `call-lua-function` mcp tool; the ingame-bridge module itself (event handling on a per-pair FIFO); the notification posting path; everything under `civ5-mod/UI/`.

## Design decisions and non-goals

- **No durable idempotency machinery.** This is a game interface: the game crashes far more often than the bridge. The one duplicate that matters — double enactment — is already prevented by `enact-agent-deal`'s transcript-level idempotency (the `deal-enacted` row is the key). A rare duplicated text row is cosmetic. Inbound duplicate *delivery* (pipe + SSE overlap) is handled cheaply by deduping on the generated event id.
- **No flush/stream generation counters.** A single human drives one panel; the bridge's per-pair FIFO plus the lua-call queue's FIFO ordering make pushes arrive in order, and `Begin` resets the pair's log. The panel's row dedupe by `ID` is the only guard kept.
...
- **Concurrency sized to human speed.** MCP notification dispatch is fire-and-forget, so the bridge serializes all handling per pair on a promise-chain FIFO (which doubles as the ordered push queue the void-returning sink callbacks need). The panel's one-in-flight-action invariant does the rest; `ThreadBusyError` surfacing as an error status is acceptable for the residual races.
- **Local legality, server enactment.** The deal screen computes per-term legality/values via direct Lua calls (the same h2h-aware `CvLuaDeal` bindings `inspect-deal.lua` uses) — no server round-trip while editing. Enactment and every transcript write stay server-side; the mod never writes transcript rows (the stage-1 writer split holds: `deal-accept`/`deal-enacted` come only from `enact-agent-deal`).
- **Declare War is a panel-local native action.** The panel's footer offers it only while the teams are at peace and `CanDeclareWar` holds; a confirmation then `Network.SendChangeWar(counterpartTeam, true)` — the same native flow the game's own UI uses. It never touches the bridge or the transcript (war state is live game state, already visible to agents through the existing knowledge tools).
...
- The shared scratch deal: `UI.GetScratchDeal()` is one object — a concurrent server-side `inspect-deal`/enact invocation can clobber an open edit. The vendored screen re-asserts players on show and re-validates before propose.
...
- Turn-boundary drift: a conversation outlives turns; legality is re-checked only at enactment (parent specs §8).
