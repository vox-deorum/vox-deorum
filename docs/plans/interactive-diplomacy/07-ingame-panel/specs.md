...
**Button existence rule: Propose Deal and Declare War exist only for a human-controlled effective seat: a normal human player or a human strategist (observer with override); never for a pure observer.** Deal cards are view-only for a pure observer, and Accept, Counter, and Reject are likewise available only to a seat holder.
...
| Deal cards | Full interaction | Full interaction on behalf of the pinned civilization | View-only |
| Propose Deal and Declare War | Present | Present | Absent |
...
The spokesperson is a Q&A voice with `get-briefing`, `send-message`, and `get-diplomatic-events`. It has no negotiator, deal, or close tools. The transport must also reject every pure-observer deal action rather than relying on tool availability alone.

Seat-holder authorization never trusts the client-supplied `PlayerID`, `AsObserver`, or display identity by itself. Before any mutation, a read-only mcp-server seat-state query reads the game's active player, observer status, and observer UI override, then derives the live effective seat and pure-observer flag from the same rule as Lua. The bridge requires the event fields to match that live state. A pure observer must identify the active observer slot and set `AsObserver: true`; normal humans and pinned strategists must identify the live effective seat and omit the field. Only the normal and pinned states establish seat authority. The fixed `{ name: 'Observer' }` identity and `civIdentity` are presentation data, never proof of a seat. Inconsistent or stale caller state fails closed before chat, greeting, or deal mutation.
...
When `AsObserver` is true and live seat attestation confirms pure observation, the bridge uses the event `PlayerID` as `callerPlayerID`, forces `agentName: 'spokesperson'`, passes `callerRole: 'Observer'` and `{ name: 'Observer' }` as the caller identity, and hard-rejects deal actions. When it is absent and attestation confirms a normal or pinned seat, the effective seat receives the normal diplomat and deal behavior. The bridge always passes the event `PlayerID` explicitly because `resolveHumanSeat` cannot infer a seat in observer or autoplay sessions.
...
- **Live effective-seat attestation and real observer endpoint admission.** Add a read-only mcp-server query for the current active player, observer state, override, effective seat, and pure-observer flag. The bridge uses it before mutations, and the transcript store uses it when validating a non-negative observer endpoint. `append-message` currently exempts only the Web's `-1` sentinel and otherwise requires both endpoints to be major civilizations. Extend that validation to admit exactly the live pure-observer slot, with role `Observer`, paired with a valid major civilization. Keep the existing `-1` behavior unchanged. Visibility remains limited to real major-civilization columns; the out-of-range observer slot is part of the ordered pair but does not create a visibility column.
...
- **Pinned-seat deal computation**: stage 02 legality, values, scratch-deal participants, and the human-side `DealPayload` actor must all use the pinned civilization rather than the active observer slot.
