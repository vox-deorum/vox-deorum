...
- **Editable proposal mounts, local editing, and server enactment.** Every mode mounts the native editor. Incoming and own proposals best-effort project normalized ordinary items into the shared scratch deal, copy their promises into the wrapper draft, and preserve the original public message. Unprojectable original terms and combination failures are surfaced with reasons, while Accept is disabled until every original ordinary item and promise is available. A normalized item-and-promise fingerprint selects the unchanged footer, Accept or Retract, or the changed footer, Counter and Reset. Reset restores the original mounted proposal; Cancel closes locally; neither reaches the driver. There is no review surface, counter sub-mode, or Back action. The scratch deal validates ordinary items through native code and presents every mounted draft: TradeLogic's in-place seams apply h2h semantics to legality reads, every native `Add*` constructor, and the three native amount-change methods. `draftItems` is a passive decode of the scratch after native editing refreshes; `draftPromises` never enters the scratch; serialization builds from the draft plus promises plus the outgoing message. Counter submits that edited draft with `expectedProposalID`. Scratch projection verifies the normalized draft before final validation and broadcasting. Stage 7.04 extends the shared proposal writer to reject structurally illegal inspected promises before archival; stage 6 rechecks everything before enactment. Server-side tools may reuse the scratch deal without destroying a mounted draft. A pending Propose or Counter keeps the editor mounted; an error restores controls without reinitializing the draft. Enactment and every transcript write stay server-side. The mod never writes transcript rows; `deal-accept` and `deal-enacted` still come only from `enact-agent-deal`.
