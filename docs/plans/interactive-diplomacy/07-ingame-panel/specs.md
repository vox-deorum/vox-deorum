...
- **Live deal actor computation**: stage 02 legality, values, scratch-deal participants, and the human-side `DealPayload` actor use the pinned civilization for a human strategist and the concrete live observer slot for a pure observer. The seat enters TradeLogic as call arguments to the parameterized `VoxDeorumOpenDeal` entry plus its `DisplayDeal` fallback; no `Game` override exists. Inspection and enactment must apply the same attestation before accepting the observer endpoint.
