...
5. **Notifications.** After any completed reply or deal outcome, call `VoxDeorumPostNotification(playerID, counterpartID, summary, message)` through `call-lua-function`. Use the counterpart leader name as the summary and a trimmed first line as the message, converting both through `markdownToPlain` before the game call. Always post; an open panel dismisses notifications for its pair. This delivers replies after the player leaves the panel, including on later turns.
