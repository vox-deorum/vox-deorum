...
A [strategist](vox-agents/strategist.md) is deciding a turn. To see the board and then act, it calls MCP tools: "what can this player build?", then "set this civ's grand strategy."
...
The round trip is complete, and the game was disturbed only when mutating its state was safe.
...
**2. The bridge fans it out.** The bridge connector sees an inbound message that matches no pending request, so it re-emits it by `type`. The event routes pick it up and broadcast it over **Server-Sent Events** to HTTP clients, over a **second outbound named pipe** to local subscribers, or both.

Both transports use the same `!@#$%^!` framing. Events are batched on a short time and count window so a busy turn doesn't flood subscribers ([bridge-service/connection.md](bridge-service/connection.md)).
...
The DLL forgets its bindings whenever the pipe drops, so the external manager re-registers everything the moment the connection returns. A bridge restart is invisible to the registered service.

Interactive diplomacy is the largest user of this path. [diplomacy.md](diplomacy.md) traces that specific round trip across every component.
...
The bridge can pause the game for specific players. When it is an LLM-controlled player's turn, the gamecore holds at a safe point until the decision comes back down, rather than letting the built-in AI act ([civ5-dll/connection.md](civ5-dll/connection.md)).
...
- The bridge is the **sole client**. It reconnects with exponential backoff, indefinitely, replaying its external-function registrations and pause state on every reconnect ([bridge-service/connection.md](bridge-service/connection.md)).
...
So you can restart the game, the bridge, or the MCP server independently, and the stack heals itself.
...
- `bridge-service/docs/protocol.md` has sequence diagrams for each flow.
- `bridge-service/docs/message-types.md` covers every JSON message type on the pipe (`lua_call`, `lua_execute`, `game_event`, external calls, registry notifications).
- `bridge-service/docs/event-pipe.md` covers the event-pipe wire format, with a client example.
- `bridge-service/docs/api-reference.md` lists every bridge HTTP endpoint, with request and response shapes.
- `mcp-server/docs/events/` holds the per-event schemas.
