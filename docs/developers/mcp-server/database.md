...
Civilization V ships its rules as SQLite. When the game loads, it writes a debug copy plus a merged localization database into its cache directory, `My Games/Sid Meier's Civilization 5/cache/` under the user's Documents folder. The MCP server opens both, read-only, through [Kysely](https://kysely.dev) over `better-sqlite3`:
...
The server never writes to either: this is the game's data, and the server is purely a consumer. Where it looks for that cache directory is configurable; see [configuration.md](configuration.md).
...
Building the mappings is treated as critical. If they can't be read, `initializeMappings` throws, because nothing downstream would make sense. That throw does not stop the server, though: the retry loop described next catches it, so the server waits and tries again rather than exiting.

## Waiting for the game to be ready

The game database files only become complete after Civilization V has launched and the mod has finished loading, so the MCP server may start before the data it needs exists.

`loadDatabaseWithRetry` handles this with one loop that covers every way startup can be too early. On each pass it checks that both database files exist and open, waits for a known late-loading table (`GreatPersons`) to appear, waits for the policy descriptions to actually resolve in the localization database, and then reads the enum mappings. Any failure along the way, including a failure to build the mappings, is caught, logged, and retried five seconds later. The loop has no attempt limit, so the server retries indefinitely.

In practice this means the server can be launched alongside the game and will simply wait for it. If the log shows the retry message repeating forever, the game has not yet produced usable data: start Civilization V once with the mod enabled and let it reach the main menu.

## How tools use it

The database-query tools (described in [tools.md](tools.md)) don't query Kysely directly in an ad-hoc way. They extend `DatabaseQueryTool`, which sits on top of the `DatabaseManager`. Each such tool provides a cached summary list of its items and a way to fetch one item's full detail; the base class adds fuzzy search and returns the full detail automatically when a search narrows to a single match. The manager's connection caching and localization caching mean repeated queries stay cheap.
