...
The reference analysis is only valid while a set of structural assumptions hold:

- every mutating tool's schema carries an explicit, clamped `PlayerID`,
- batched Lua scripts touch a single player rather than iterating all of them,
- the Vox Deorum setters don't fire game-event hooks, and
- the bridge runs on localhost without auth, which is what makes `lua-executor` and the global pause acceptable.

These invariants are written out in `mcp-server/docs/tactical-ai-influence.md`. If a change to the server or the DLL violates one, the downstream impact claims need revisiting, and that file is where to check.
