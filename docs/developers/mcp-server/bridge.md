...
The queue is also where the server protects itself from getting overwhelmed. After draining a batch, if 25 or more calls are still waiting, the manager **pauses the game** so it can catch up, then **resumes** once the backlog falls below that mark. This auto-pause/resume is tracked as a single overflow state, so the server doesn't thrash between pausing and resuming on every batch.
...
Pause and resume themselves go out on a **fast path**, a separate connection pool of 5 sockets reserved for low-latency operations, alongside the standard pool of 50 that carries everything else. A control action therefore doesn't queue up behind a pile of pending reads. The same fast path carries the per-player auto-pause registration and the production-mode toggle described below.
...
These map directly onto the bridge's pause manager; the game-side semantics live in [connection.md](../bridge-service/connection.md). The tools that drive them are `pause-game`, `resume-game`, and `set-production-mode` (see [tools.md](tools.md)).
...
- On a stream disconnect it schedules a reconnect one second later, retrying the event pipe first and then SSE. The delay is a fixed constant in `bridge/manager.ts`, not a configurable setting.
