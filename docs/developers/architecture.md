...
Each layer is independently runnable and restartable. The bridge survives the game restarting, the MCP server survives the bridge bouncing, and the agents survive all of it. That resilience is a deliberate property of the boundaries. See [Why the layers exist](#why-the-layers-exist).
...
The [mcp-server](mcp-server/overview.md) is the bridge's primary consumer and the point where the AI side meets the game. It exposes Civ V's state and controls as 43 [Model Context Protocol](https://modelcontextprotocol.io) tools registered in `mcp-server/src/tools/index.ts`, covering questions like "what can this player research?" and actions like "set this civ's grand strategy to conquest".
...
The framework is provider-agnostic (OpenAI, Anthropic, Google, AWS Bedrock, OpenRouter, and local OpenAI-compatible endpoints) with per-agent model assignment. A [Vue web UI](vox-agents/ui.md) ships alongside it for driving and watching sessions.
...
- **DLL ↔ Bridge (named pipe).** The game is a 32-bit Windows process with a private IPC mechanism and hard rules about thread-safety and save serialization. Sealing that complexity behind one pipe, with one server and one client, means everything above it can be ordinary cross-platform Node.js.
...
- [Setup](setup.md) builds the stack from source and runs it end to end.
- [Protocol](protocol.md) walks the full message path, layer by layer.
- [Diplomacy](diplomacy.md) follows one interactive round trip across every component, from the game's diplomacy screen to the model and back.
- [Operations](operations.md) covers the maintenance scripts, log locations, and debugging a running stack.
- [Testing](testing.md) and [Releasing](releasing.md) explain how the project is tested and shipped.
- The five component folders linked above go deep on any single layer.
