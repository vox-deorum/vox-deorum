...
Prompt-mode models don't call tools natively; the tool-rescue middleware strips native tools and injects a JSON tool-call protocol as a system message. For claude-code models that protocol — and the agent's own system prose — is reframed from "tool" to "action" (its CLI persona reasons in terms of actions, and it keeps the game tools from reading as the CLI's native tools). Each replayed turn records this as telemetry: `step.tool_framing` (`'tool'` or `'action'`) always, and `step.tool_prompt` (the injected protocol, in vanilla `'tool'` wording) only when the turn was adapted to `'action'`.
