...
The current pin is `codex-openai-proxy@0.1.0-rc.25`, which bundles `@openai/codex@0.153.4`.
...
Continuation admission belongs to the proxy. Vox Agents threads the previous step's response id as `previous_response_id` between the steps of one agent run, so the proxy prefers native thread continuation over replaying a fresh thread. When the requested continuation is unavailable, the proxy executes the supplied complete transcript on a fresh Codex thread and reports `x_codex.threadReused: false` instead of rejecting the request, so the adapter handles no selector errors. Vox Agents always sends the full client-visible transcript with provider-executed activity stripped from replay, which keeps a fallback transcript completely paired. Every Codex model step records the admitted outcome as `host.thread_reuse` telemetry: `reused` when the proxy continued a native thread, `tried_failed` when a selector was sent but a fresh thread executed, and `fresh` when no selector was sent. The adapter still marks deterministic typed errors such as `tool_results_required`, `tool_results_without_pending_call`, `thread_not_resumable`, and `duplicate_tool_call_id` as non-retryable in `codex-response.ts`, while transient states such as `thread_busy` stay retryable.
