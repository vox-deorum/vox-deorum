...
| Agent chat | `agent.ts` | List registered agents; create chat threads against exactly one live context (an in-game [envoy](envoy.md) conversation) or database path (a [telepathist](telepathist.md) session with a read-only source connection and a context owned by that thread); and the unified `POST /api/agents/message` endpoint, which executes the thread's agent and streams text, reasoning, tool-call, and tool-result events back as SSE. |
