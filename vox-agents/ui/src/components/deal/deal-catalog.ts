 ...
 *
 * Turns one side's `NormalizedSideRange` (+ default duration, current deal) into the ordered,
 ...
 * with sensible defaults; amounts / quantities / durations / targets are then edited on the
 * central offer rows. Singleton terms already on the table show selected + disabled.
 */
 ...
 * duration for duration-bearing types; gold uses the (already capped) seed amount. Everything
 * is editable afterwards on the central offer row.
 */
export function defaultItemFor(
  ...
  switch (itemType) {
    ...
    case 'GOLD_PER_TURN':
      return { ...base, amount: 0, duration: ctx.defaultDuration };
    case 'RESOURCES':
      return { ...base, resourceID: ctx.resourceID, quantity: 1, duration: ctx.defaultDuration };
    case 'CITIES':
      return { ...base, cityID: ctx.cityID };
    case 'TECHS':
      return { ...base, techID: ctx.techID };
    case 'THIRD_PARTY_PEACE':
      return { ...base, thirdPartyTeamID: ctx.thirdPartyTeamID, duration: ctx.defaultDuration };
    ...
    default:
      // Single-shot toggles carry no extra data.
      return base;
  }
}
...
export function buildSideCatalog(args: {
  ...
  const { ownerID, otherID, range, currentItems, currentPromises, defaultDuration, promiseTargets } = args;
  ...
  if (range) {
    ...
    gold.push(itemRow(
      ...
      defaultItemFor('GOLD_PER_TURN', ownerID, otherID, { defaultDuration })
    ));
  }
    ...
    (range?.resources ?? [])
      ...
      .map((r) =>
        itemRow(
          ...
          defaultItemFor('RESOURCES', ownerID, otherID, { resourceID: r.resourceID, defaultDuration }),
          `≤ ${r.quantityAvailable}`
        )
      );
  ...
  const peaceTargets: InventoryTarget[] = (range?.thirdPartyPeace ?? []).map((t) => ({
    ...
    addPayload: { kind: 'item', item: defaultItemFor('THIRD_PARTY_PEACE', ownerID, otherID, { thirdPartyTeamID: t.teamID, defaultDuration }) },
  }));
