...
| `relay-message` | Relay a report visible to the receiving leader; `Importance` 7+ interrupts important-event pacing. Subject IDs are stored separately and resolved to names when retrieving formatted diplomatic history | `PlayerID`, `FromPlayerID`, `AboutPlayerIDs?`, `Message`: "Diplomatic"/"Intelligence"/"Rumor", `Content` (1-4000 characters), `Confidence` (0-9), `Importance` (0-9), `Categories` (any of Diplomacy, Military, Economy, Others), `Memo` (1-500 characters) |
